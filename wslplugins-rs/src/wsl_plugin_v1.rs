use crate::{
  distribution_information::DistributionInformation, offline_distribution_information::OfflineDistributionInformation, wsl_session_information::WSLSessionInformation, wsl_vm_creation_settings::WSLVmCreationSettings, ApiV1
};
...
pub trait WSLPluginV1: Sized {
  ...
  /// Introduced in 2.1.2
  #[allow(unused_variables)]
  fn on_distribution_registered(&self, session: &WSLSessionInformation, distribution: &OfflineDistributionInformation) -> Result<()> {
    Ok(())
  }
  ...
  /// Introduced in 2.1.2
  #[allow(unused_variables)]
  fn on_distribution_unregistered(&self, session: &WSLSessionInformation, distribution: &OfflineDistributionInformation) -> Result<()>{
    Ok(())
  }
}